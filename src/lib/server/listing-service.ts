...
const listingFormBaseSchema = z.object({
  ...
  buildingName: z.string().trim().max(120, "Building name must be 120 characters or fewer").optional().default(""),
...
export function buildListingUpdateData(data: ListingFormData) {
  return {
    ...
    genderPreference: toPrismaGenderPreference(data.genderPreference),
    buildingName: data.buildingName.trim() ? data.buildingName.trim() : null,
